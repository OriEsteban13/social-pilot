...
- ~~`GET /v2/settings/brands/{blogId}/connections`~~ — **devuelve 404, parece retirada.** Las redes conectadas de una marca están en realidad en `data.networksData` (objeto indexado por nombre de red) dentro de `GET /v2/settings/brands/{blogId}` (sin sufijo), que sí responde 200. `findConnection()` en `adapter.ts` ya usa esta ruta corregida. Solo se ha podido confirmar la forma de `networksData` **vacío** (`{}`, cuenta sin ninguna red conectada todavía) — la forma exacta de una entrada con una red real conectada sigue sin confirmar; revisar `connectAccount`/`validateConnection` en cuanto haya al menos una red conectada en app.metricool.com.
...
- **AI Provider** (`src/server/ai/`): interfaz común (`AIProvider`). Implementación por defecto `MockAIProvider` (plantillas + datos reales del Brand Brain, sin llamadas externas). Las reglas de marca/idioma compartidas por los proveedores reales viven en `src/server/ai/providers/shared-prompts.ts`. Dos implementaciones reales:
  ...
  - `GroqAIProvider` (`src/server/ai/providers/groq.ts`), activable con `AI_PROVIDER=groq` + `GROQ_API_KEY` — inferencia muy rápida/barata sobre modelos open-weight de terceros vía la API de Groq (compatible con OpenAI). Modelo por defecto `moonshotai/kimi-k2-instruct-0905` (configurable con `GROQ_MODEL`), uno de los pocos con cumplimiento estricto de JSON Schema (`strict: true`) confirmado en la documentación de Groq; para los demás modelos, Groq solo garantiza el esquema en modo `strict: false` (best-effort), por lo que además de pedirlo se valida la respuesta con el propio esquema Zod (`schema.parse(...)`) antes de usarla. Reutiliza los mismos esquemas Zod que Anthropic (`anthropic-schemas.ts`), convertidos a JSON Schema con `z.toJSONSchema()` (nativo de Zod 4). Calidad de redacción algo por debajo de Claude al ser un modelo open-weight — revisar el contenido generado con más atención, sobre todo al principio.
