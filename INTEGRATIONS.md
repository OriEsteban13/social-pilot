...
- **AI Provider** (`src/server/ai/`): interfaz común (`AIProvider`). Implementación por defecto `MockAIProvider` (plantillas + datos reales del Brand Brain, sin llamadas externas). Las reglas de marca/idioma compartidas por los proveedores reales viven en `src/server/ai/providers/shared-prompts.ts`. Dos implementaciones reales:
  ...
  - `GroqAIProvider` (`src/server/ai/providers/groq.ts`), activable con `AI_PROVIDER=groq` + `GROQ_API_KEY` — inferencia muy rápida/barata sobre modelos open-weight de terceros vía la API de Groq (compatible con OpenAI). Modelo por defecto `llama-3.3-70b-versatile` (configurable con `GROQ_MODEL`) — de propósito general, disponible en cualquier cuenta sin habilitación aparte. Se probó primero `moonshotai/kimi-k2-instruct-0905` (uno de los pocos con `strict: true` confirmado en la documentación de Groq) pero devolvió 404 `model_not_found` contra una cuenta real; comprobar el catálogo vigente en `console.groq.com/docs/models` antes de cambiar `GROQ_MODEL`. Groq solo garantiza el esquema en modo `strict: false` (best-effort) salvo en el subconjunto de modelos con soporte estricto, por lo que además de pedirlo se valida la respuesta con el propio esquema Zod (`schema.parse(...)`) antes de usarla. Reutiliza los mismos esquemas Zod que Anthropic (`anthropic-schemas.ts`), convertidos a JSON Schema con `z.toJSONSchema()` (nativo de Zod 4). Calidad de redacción algo por debajo de Claude al ser un modelo open-weight — revisar el contenido generado con más atención, sobre todo al principio.
