...
- Conexiones de una marca: `GET /v2/settings/brands/{blogId}/connections`.
...
**No verificado contra una cuenta real** (sin credenciales en este entorno): la forma exacta de los campos de respuesta (qué clave contiene el id del post creado, la forma exacta de una "connection", los nombres de campo de las métricas de analítica). El adaptador parsea estos campos de forma defensiva (varios nombres candidatos) — revisar y ajustar en cuanto haya una cuenta real conectada, antes de confiar en producción. `publishPost()` tampoco puede devolver la URL final de la red social de forma síncrona (Metricool la publica poco después de la llamada, de forma asíncrona en su propio scheduler); de momento se devuelve un enlace al planner de Metricool en su lugar.
